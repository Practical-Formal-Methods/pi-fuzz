...
import mod_gym
from mod_gym.gym.wrappers import GrayScaleObservation
...
def test_gray_scale_observation(env_id, keep_dim):
    gray_env = AtariPreprocessing(mod_gym.make(env_id), screen_size=84, grayscale_obs=True)
    rgb_env = AtariPreprocessing(mod_gym.make(env_id), screen_size=84, grayscale_obs=False)
