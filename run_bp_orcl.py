...
def test_policy(env_identifier, fuzz_type, agent_path, bug_type, coverage, coverage_thold, r_seed, fuzz_mut_bdgt, orcl_mut_bdgt, delta, sp, pp):
    ...
    pop_summ, pool = pickle.load(open(pp, "rb"))
    fuzzer.pool = pool
    fuzzer.total_trials = pop_summ[-1][0]
...
parser.add_argument("-SP", "--seed_pool", type=int,  required=True)
parser.add_argument("-PP", "--path_pool",  required=True)

args = parser.parse_args()
path_to_pool = args.path_pool
seed_ptr = args.seed_pool
...
test_out = test_policy(env_iden, fuzz_type, agent_path, bug_type, coverage, coverage_thold, rand_seed, fuzz_mut_bdgt, orcl_mut_bdgt, delta, seed_ptr, path_to_pool)
